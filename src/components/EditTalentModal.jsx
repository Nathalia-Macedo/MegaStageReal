...
export default function EditTalentModal({ isOpen, onClose, talentId, onSave }) {
  ...
  const handleDeletePreviousJob = (jobId) => {
    setConfirmationModal({
      ...
      message: "Tem certeza que deseja excluir este trabalho anterior? Esta ação não pode ser desfeita.",
      loading: false,
    })
  }
  ...
  const confirmDelete = async () => {
    ...
    try {
      if (itemType === "video") {
        ...
        toast.success("Trabalho anterior excluído com sucesso!")
  ...
  const fetchTalentVideosList = async (id) => {
    ...
    try {
      ...
      // Não mostrar erro se for apenas porque não há vídeos
  ...
  // Função para buscar trabalhos anteriores
  ...
  const handleVideoUpload = async (files) => {
    ...
    try {
      ...
      toast.info(`Enviando ${files.length} vídeo(s)...`, {
        autoClose: false,
        toastId,
      })
      ...
      // Marcar formulário como alterado para habilitar botão salvar
      setFormTouched(true)
  ...
  const renderPreviousJobsSection = () => (
    <ModalSection activeSection={activeSection} sectionKey="previousJobs">
      {/* Área para adicionar novo trabalho anterior */}
        ...
        {loadingPreviousJobs ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-pink-500 dark:text-pink-500 text-sm">
              Use o formulário acima para adicionar trabalhos anteriores
            </p>
          </div>
        ) : (
          <div className="space-y-4">
