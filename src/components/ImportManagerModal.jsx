...
export default function ImportManagerModal({ isOpen, onClose }) {
  ...
  const handleImport = async () => {
    ...
    try {
      console.log("🚀 Iniciando importação:", importType)
      const isIncremental = importType === "incremental"
      const result = await importFromManager(isIncremental)

      console.log("📥 Resultado da importação recebido:", result)

      // Processar o resultado para garantir que temos os dados corretos
      let processedResult = {
        success: true,
      ...
      if (result) {
        // Se result é um objeto com as propriedades esperadas
        if (typeof result === "object") {
          processedResult = {
            success: result.success !== false,
            message: result.message || processedResult.message,
            count: result.count || result.imported || 0,
          }
        }
        // Se result é um número (quantidade importada)
        else if (typeof result === "number") {
          processedResult.count = result
          processedResult.message = `${result} ${result === 1 ? "talento importado" : "talentos importados"} com sucesso!`
        }
        // Se result é uma string (mensagem)
        else if (typeof result === "string") {
          // Tentar extrair número da mensagem
          const numberMatch = result.match(/(\d+)/)
          if (numberMatch) {
            processedResult.count = Number.parseInt(numberMatch[1])
          }
          processedResult.message = result
        }
      }

      console.log("✅ Resultado processado:", processedResult)

      setImportResult(processedResult)
      setShowConfetti(processedResult.count > 0)

      toast.success(processedResult.message, {
        icon: <CheckCircle className="text-green-500" size={18} />,
      })
    } catch (error) {
      console.error("❌ Erro na importação:", error)
      const errorMessage = error.message || "Erro ao importar talentos do Manager"
      setImportError(errorMessage)
      toast.error(`Erro ao importar talentos: ${errorMessage}`, {
        icon: <AlertCircle className="text-red-500" size={18} />,
      })
    } finally {
      setIsImporting(false)
                        ...
                        <div className="flex-1">
                          ...
                          {importResult.count === 0 && (
                            <div className="mt-2 text-sm text-green-600 dark:text-green-400">
                              ℹ️ Nenhum novo talento encontrado para importar
                            </div>
                          )}
                        </div>
                      </div>
